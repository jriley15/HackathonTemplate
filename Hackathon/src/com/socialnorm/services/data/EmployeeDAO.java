...
public class EmployeeDAO implements IEmployeeDAO
{
	...
	public List<EmployeeModel> getEmployees() 
	{
		...
		try 
		{
			...
			while(rs.next())
			{
				EmployeeModel employee = new EmployeeModel(rs.getString("FIRSTNAME"), rs.getString("LASTNAME"), rs.getString("EMAIL"), rs.getString("PHONE"), rs.getString("EMPLOYEEID"), rs.getString("ADMIN"), rs.getString("ACTIVE"), rs.getString("TERMINATED"));
	...
	public boolean updateEmployees(List<EmployeeModel> employees) 
	{
		...
		try 
		{
			...
			while(i < employees.size())
			{
				...
				// n1euzrfjibaye0bl
				// second query for inserting the topic into the posts table
				String query = "UPDATE activity2.authemployee SET ADMIN = ?, ACTIVE = ?, TERMINATED = ? WHERE EMPLOYEEID = ?";
